...

    {
       ...
       //------------------------THONG TIN PHONG------------------------------------------
        ...
        private void btn_lhd_Click(object sender, EventArgs e)
        {
            ...
            //hien thi data
            string sql_qldt = "select hd.hd_ma, hd.hd_ngaylap, kh.KH_TEN, kh.kh_cccd, kh.kh_sdt, lkh.lkh_ten as LKH , p.ten_phong, lp.ten_loai_phong as LP, dv.dv_ten as DICH_VU, lp.dongia as DONGIA_PHONG, dv.dv_dongia as DG_DICHVU, hd.hd_thanhtien, httt.httt_ten, nv.nv_ten as TENNHANVIEN from hoa_don hd, khachhang kh, loaikhachhang lkh , phong p, loai_phong lp, dich_vu dv, hinh_thuc_thanh_toan httt, nhan_vien nv where hd.kh_cccd = kh.kh_cccd and kh.lkh_ma = lkh.lkh_ma and hd.ma_phong = p.ma_phong and p.ma_loaiphong = lp.ma_loaiphong and hd.dv_ma = dv.dv_ma and hd.httt_ma = httt.httt_ma and hd.nv_ma = nv.nv_ma";
        ...
        private void btn_qldt_Click(object sender, EventArgs e)
        {
            ...
            string sql_qldt = "select * from hoa_don";
        ...
        private void admin_Load(object sender, EventArgs e)
        {
            ...
            string sql_getht = "select nv_ma , nv_ten from nhan_vien where nv_ma ='" + tennguoidung + "'";
            SqlCommand comd = new SqlCommand(sql_getht, conn);
            SqlDataReader reader = comd.ExecuteReader();
            if (reader.Read())
            {
                label6.Text = "Xin Chào, " + reader.GetValue(1).ToString();
                manv = reader.GetValue(0).ToString();
            }
            else
            {
                MessageBox.Show("Thất bại mất rồi");
            }
            reader.Close();
        ...
        private void ttp_search_TextChanged(object sender, EventArgs e)
        {
           string keysearch = ttp_search.Text;
            string search = "SELECT p.ma_phong as MA_PHONG, lp.ma_loaiphong as MA_LOAI_PHONG, p.ten_phong as TEN_PHONG, lp.ten_loai_phong as LOAI_PHONG, lp.dongia as DON_GIA_PHONG, tt.ten_trangthai as TINH_TRANG, p.hinhanh FROM trang_thai tt, phong p, loai_phong lp where   p.ma_loaiphong = lp.ma_loaiphong and tt.ma_trangthai = p.ma_trangthai and ( p.ten_phong like '%" + keysearch + "%' OR lp.ten_loai_phong like '%" + keysearch + "%' OR lp.dongia = '%" + keysearch + "%' OR tt.ten_trangthai like '%" + keysearch + "%' )";
        ...
        private void qldt_search_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if(e.KeyCode == Keys.Enter)
            {
                ...
                string sql_searchqldt = "select * from hoa_don where hd_ma like '%"+keysearchqldt+"%' OR hd_ngaylap like '%"+keysearchqldt+"%'OR kh_cccd like '%"+keysearchqldt+"%' ";
        ...
        private void qldt_lietke_Click(object sender, EventArgs e)
        {
            string tungay = qldt_timefrom.Value.ToString("MM-dd-yyyy");
            string denngay = qldt_timeto.Value.ToString("MM-dd-yyyy");
            string sql_lietke = "select * from hoa_don where hd_ngaylap between '" + tungay + "' and '" + denngay + "' ";
        ...
        private void lhd_search_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                string keysr = lhd_search.Text;
                string Sql_lll = "select hd.hd_ma, hd.hd_ngaylap, kh.KH_TEN, kh.kh_cccd, kh.kh_sdt, lkh.lkh_ten as LKH , p.ten_phong, lp.ten_loai_phong as LP, dv.dv_ten as DICH_VU, lp.dongia as DONGIA_PHONG, dv.dv_dongia as DG_DICHVU, hd.hd_thanhtien, httt.httt_ten, nv.nv_ten as TENNHANVIEN from hoa_don hd, khachhang kh, loaikhachhang lkh , phong p, loai_phong lp, dich_vu dv, hinh_thuc_thanh_toan httt, nhan_vien nv where hd.kh_cccd = kh.kh_cccd and kh.lkh_ma = lkh.lkh_ma and hd.ma_phong = p.ma_phong and p.ma_loaiphong = lp.ma_loaiphong and hd.dv_ma = dv.dv_ma and hd.httt_ma = httt.httt_ma and hd.nv_ma = nv.nv_ma AND (hd_ma LIKE '%" + keysr + "%' OR kh.kh_ten LIKE '%" + keysr + "%' OR kh.kh_cccd LIKE '%" + keysr + "%' OR kh.kh_sdt LIKE '%" + keysr + "%' OR lkh.lkh_ten like '%" + keysr + "%' OR dv.dv_ten like '%"+ keysr+"%' OR lp.dongia like '%"+ keysr+ "%' OR dv.dv_dongia like '%"+ keysr+ "%' OR hd.hd_thanhtien like '%"+ keysr+ "%' OR httt.httt_ten like '%"+ keysr+ "%' OR nv.nv_ten like '%"+ keysr+"%') ";
        ...
        {   
            ...
            cb_lkh.Text = dgv_khachhang.Rows[i].Cells[1].Value.ToString();
            ...
            tb_sdt_kh.Text = dgv_khachhang.Rows[i].Cells[6].Value.ToString();
            tb_diachi_kh.Text = dgv_khachhang.Rows[i].Cells[7].Value.ToString();
...

        {
            ...
            pd_nl.Text = dgv_phieudatphong.Rows[k].Cells[5].Value.ToString();
        ...
        private void nv_data_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            string path = nv_data.Rows[e.RowIndex].Cells[7].Value.ToString();
       
            try
            {
                nv_pic.Image = Image.FromFile(path);
            }
            catch (FileNotFoundException)
            {
                // Handle the exception by displaying a default image or showing an error message
        ...
        private void cb_pd_gioitinhkhachhang_SelectedIndexChanged(object sender, EventArgs e)
        {

        }

        private void tb_kh_search_KeyDown(object sender, KeyEventArgs e)
        {

            if (e.KeyCode == Keys.Enter)
            {
                string keysearch = tb_kh_search.Text;
                //command = connection.CreateCommand();
                string sql = "SELECT * FROM KHACHHANG WHERE kh_ma LIKE '%" + keysearch + "%' OR kh_ten LIKE '%" + keysearch + "%' ";
                
                func.HienthiDulieuDG(dgv_khachhang, sql, conn);
            }
            
        }

        private void pd_search_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                //command = connection.CreateCommand();
                string sql = "SELECT * FROM PHIEUDATPHONG WHERE pd_ma LIKE '%" + pd_search.Text + "%' OR ma_phong LIKE '%" + pd_search.Text + "%' ";
                function fun = new function();
                fun.HienthiDulieuDG(dgv_phieudatphong, sql, conn);

            }
        }

        private void ttp_data_Click(object sender, EventArgs e)
        {
            int j;
            j = ttp_data.CurrentRow.Index;
            ttp_mp.Text = ttp_data.Rows[j].Cells[0].Value.ToString();
            ttp_malp.Text = ttp_data.Rows[j].Cells[1].Value.ToString();
            ttp_tenphong.Text = ttp_data.Rows[j].Cells[2].Value.ToString();
            ttp_lp.Text = ttp_data.Rows[j].Cells[3].Value.ToString();
            ttp_dg.Text = ttp_data.Rows[j].Cells[4].Value.ToString();
            ttp_ttrang.Text = ttp_data.Rows[j].Cells[5].Value.ToString();
            //ttp_linkanh.Text = ttp_data.Rows[e.RowIndex].Cells[6].Value.ToString();

            string path = ttp_data.Rows[j].Cells[6].Value.ToString();
            try
            {
                ttp_picture.Image = Image.FromFile(path);
            }
        ...
        private void nv_search_TextChanged(object sender, EventArgs e)
        {
            string keysr = nv_search.Text;
            string sqlnv = "select NV_MA, NV_TEN, NV_CCCD, NV_SDT, NV_NGAYSINH, NV_GIOITINH, NV_DIACHI, NV_HINHANH from nhan_vien where (nv_ma like '%"+keysr+"%' OR nv_ten like N'%" + keysr + "%' OR nv_cccd like '%" + keysr + "%' OR nv_diachi like N'%" + keysr + "' OR nv_sdt like '%" + keysr + "')";
            func.HienthiDulieuDG(nv_data, sqlnv, conn);
        }

        private void nv_search_KeyDown(object sender, KeyEventArgs e)
        {

        }

        private void pd_search_TextChanged(object sender, EventArgs e)
        {

        }

        private void tb_kh_search_TextChanged(object sender, EventArgs e)
        {

        }

        private void qldv_search_TextChanged(object sender, EventArgs e)
        {

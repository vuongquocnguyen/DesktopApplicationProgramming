...
namespace quanlykhachsan
{
    public partial class updatephong : Form
    {
        ...
        private void updatep_save_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    string linkanh = upnv_link.Text;

                    string sql_update = "UPDATE phong SET ma_loaiphong = '" + umlp + "', ten_phong= '" + utp + "', ma_trangthai = '" + uttp + "', hinhanh = '" + linkanh + "' where (ma_phong = '" + ump + "') " ;
                    
                    func.CapNhat(sql_update, conn);
                    func.HienthiDulieuDG(upp_data, "SELECT p.ma_phong, lp.ma_loaiphong, p.ten_phong, lp.ten_loai_phong, lp.dongia, tt.ten_trangthai, p.hinhanh from trang_thai tt, phong p, loai_phong lp where tt.ma_trangthai = p.ma_trangthai and p.ma_loaiphong = lp.ma_loaiphong", conn);
        ...
        private void uploads_tp_Click(object sender, EventArgs e)
        {
            ...
            {
                upp_picture.Image = new Bitmap(op.FileName);
                file = op.FileName;
                upnv_link.Text = file;
            }
        }
        ...
        private void upp_data_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            upnv_link.Text = upp_data.Rows[e.RowIndex].Cells[6].Value.ToString();
            string path = upp_data.Rows[e.RowIndex].Cells[6].Value.ToString();
            try
            {
                upp_picture.Image = Image.FromFile(path);
            }
